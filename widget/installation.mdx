...
The widget stays hidden until `identify()` is called. With no arguments, it starts an anonymous session so the widget appears for every visitor. To attribute feedback to logged-in users, see [Identify users](/widget/identify-users).
...
For unauthenticated visitors, call with no arguments:
...
Clear the current identity and hide the widget. Call this from your logout handler.
