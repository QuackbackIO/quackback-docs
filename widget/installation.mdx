...
| `buttonColor` | string (hex) | Theme primary | Override the launcher button background color |
| `launcher` | boolean | `true` | Set to `false` to hide the default floating button |
| `tabs` | object | All enabled | Control visible tabs: `{ feedback: true, changelog: true, help: false }` |
| `locale` | string | Auto-detected | Force a locale: `"en"`, `"fr"`, `"de"`, `"es"`, `"ar"`, `"ru"`, `"pt-br"`, `"zh-cn"`, or `"zh-tw"`. Omit to auto-detect from the browser |
