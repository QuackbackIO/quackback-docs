...
| `identity` | object | - | Bundle identity at init time. Accepts the same payloads as [`identify`](#identify) — `{ id, email, name }`, `{ ssoToken }`, or `{ anonymous: true }`. Equivalent to calling `init` and then `identify` back-to-back |
