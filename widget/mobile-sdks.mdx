...
// omit identify for anonymous visitors
...
Quackback.configure(
    ...
    identity: .ssoToken(fetchedSsoToken)
)
// Omit the `identity` parameter for anonymous sessions.
```
...
// After user signs in — omit identify for anonymous visitors
Quackback.identify(ssoToken = fetchedSsoToken)
...
Quackback.configure(
    ...
    identity = Identity.SsoToken(fetchedSsoToken)
)
// Omit the `identity` parameter for anonymous sessions.
