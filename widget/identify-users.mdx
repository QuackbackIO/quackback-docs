...
1. Get the host signing secret (agent prompt on [Install](/widget/installation), or Reveal / Copy). Any server-only env name works.
...
Identify signs the visitor in as a **customer in the widget** only. That sign-in never unlocks the admin dashboard — even if the same email is a workspace admin or member. Team members can identify and use the widget the way a customer would; their dashboard name and avatar are not overwritten by the host-app token. See [Session types](/auth/overview#session-types).
...
If you run a private portal, a verified widget visitor can open the portal already signed in — no password, no second sign-up.
...
They stay a **customer** on the portal. Opening the portal from the widget does not grant admin access. Workspace teammates can identify in the widget, but **Go to portal** does not sign them in — they land on the portal sign-in screen (or stay on an existing dashboard login). See [Control portal access](/admin/portal-auth#widget-sign-in) and [Session types](/auth/overview#session-types).
