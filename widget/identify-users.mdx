...
1. Copy the **signing secret** from **Admin → Settings → Widget → Install**
2. Store it in your product's server-side secret store (example: `WIDGET_SIGNING_SECRET`). This is not a Quackback Cloud or self-host setting — Cloud and self-host use the same admin copy path.
...
<Warning>
Never put the signing secret in client-side code, commits, or logs. Sign tokens on your server only.
</Warning>
