...
- **Anonymously** — don't identify them yet. The widget stays visible, and asks for an email inline the first time they post.
...
For visitors who aren't signed in, start with:

```js
Quackback("identify", { anonymous: true });
```

The widget stays visible, and the session is created the first time the user does something — a vote, comment, or post. If they later submit their email or you pass an `ssoToken`, everything they did anonymously moves across automatically.
