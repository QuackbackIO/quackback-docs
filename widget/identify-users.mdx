...
There are three ways to identify a user:

- **With their details** — the simplest option. Pass `id`, `email`, and `name` directly from the client. Works out of the box.
...
- **Anonymously** — call `Quackback("identify")` with no arguments. The widget appears, and asks for an email inline the first time the user posts.

You can mix and match — for example, identify signed-in users with an `ssoToken` and everyone else anonymously.
...
For visitors who aren't signed in, call `identify` with no arguments:

```js
Quackback("identify");
```

The widget appears, and the session is created the first time the user does something — a vote, comment, or post. If they later submit their email or you pass an `ssoToken`, everything they did anonymously moves across automatically.

<Tip>
A common pattern: identify signed-in users with their details (or an `ssoToken`), and fall back to anonymous for everyone else. That way the widget is always visible, and feedback gets attributed as soon as you know who someone is.
</Tip>
...
Call `logout` to reset the widget to an unidentified state. Put this in your logout handler.
