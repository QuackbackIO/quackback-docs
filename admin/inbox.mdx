...
description: "Filter, triage, and respond to feedback from the Feedback area."
icon: "chat-bubble-left"
---

# Triage feedback

Stop context-switching between tools. The Feedback area is your command center. See everything users are asking for, triage quickly, and respond without leaving the page.

<Note>
This page covers feedback (posts, votes, comments). Conversations and support tickets live in a separate support inbox, see [Support inbox](/support/inbox).
</Note>
...
- Move promising ideas to a status that puts them on your public [roadmap](/admin/roadmap)
...
- Properties panel (Status, Board, Tags, Owner, ETA)
- Author information
...
### Set a target date (ETA)

1. Open a post in the detail page
2. In the Properties panel, click **Set ETA** and choose a month
3. Clear it the same way if plans change

Roadmaps are derived from post fields like status, board, tags, and eta, so a post shows up on a [roadmap](/admin/roadmap) automatically once it matches that roadmap's filter. There's no separate step to add a post to a roadmap.
...
Filter the feedback list by owner to see your assigned posts or find unassigned work.
...
If the post has linked issues in an [integration](/integrations/overview) like Linear, GitHub, or Jira, you'll see checkboxes to archive or close those issues at the same time. Each integration's default behavior is configurable under **Admin → Settings → Integrations**.
