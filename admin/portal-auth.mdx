...
Segment access works for any authenticated visitor whose principal is a member of one of the chosen segments. Unlike the domain channel, segment access does **not** require a verified email — segment membership is admin-curated, so the [segment evaluator](/admin/segments#dynamic-segments) is the trust anchor.
