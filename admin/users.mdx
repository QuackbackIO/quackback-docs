...
- Filter by [segment](/admin/segments), verification status, or sign-up date
- Search by name or email
- Send and manage [portal invites](#invitations)
...
The sidebar splits users into lifecycle views so you're never scanning one giant list for a different kind of person:

| View | Shows |
|------|-------|
| **All users** | Identified users - anyone with a verified account |
| **All leads** | Engaged anonymous visitors - people who've voted, commented, or posted without creating an account |
| **All companies** | The company directory. See [Manage companies](/admin/companies) |

<Note>
A lead is someone who's engaged with your feedback (a vote, comment, or post) while still anonymous. If they later sign up, they move from All leads into All users automatically.
</Note>
...
| Name | Display name |
| Email | Email address (may be blank for users created via integrations) |
...
Filter users by [segment](/admin/segments). Select a segment from the filter sidebar to view only users in that group.
...
| Last Active | Most recent session or activity first |
...
| Action | What it does |
|--------|--------------|
| Copy link | Copies the same magic link from the email so you can share it directly. |
| Resend | Sends a fresh email with the same link. |
| Revoke | Cancels the invite immediately. The link stops working. |

Each invite is valid for 14 days. Past that, the daily sweep marks it **Expired** and emits a `portal.invite.expired` event to the [audit log](/admin/security#audit-log).
...
For setup guidance on the portal-side controls (visibility, segments, widget sign-in), see [Control portal access](/admin/portal-auth#portal-access).

## Access Control

| Role | Can View Users | Can Remove Users | Can Invite |
|------|----------------|------------------|------------|
| Admin | Yes | Yes | Yes |
| Member | Yes | No | No |
...
- [Manage companies](/admin/companies) - View the company directory and custom company attributes
- [Segments](/admin/segments) - Group users for targeted feedback analysis
