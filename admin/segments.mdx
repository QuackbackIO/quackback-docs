...
Each attribute exposes the operators its data type supports. Numbers add `greater than`, `at least`, `less than`, `at most`. Strings add `contains`, `starts with`, `ends with`. Every attribute also supports `is set` and `is not set` — handy for finding users with a missing locale or no `plan` value yet.
...
- **Per-board access** — limit a board's audience to one or more segments. See [Board audience](/admin/boards#access-tab).
