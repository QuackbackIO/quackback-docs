...
The only exception is AI features. When enabled, specific data is sent to your configured AI provider for processing. See [AI data handling](#ai-data-handling) below.
...
<Warning>
Set a strong, unique `SECRET_KEY` (at least 32 characters) before connecting any integrations. Changing this key later invalidates all stored tokens and sessions.
</Warning>
...
When you enable AI features, Quackback sends data to the AI provider you configure via `OPENAI_BASE_URL`: OpenAI, OpenRouter, a gateway, or a local server. See [Use AI-powered features](/admin/ai-features) for configuration.
...
When enabled, the portal tracker honors **Do Not Track** and **Global Privacy Control** browser signals, it stops collecting for any visitor who has either one set. See [Visitor analytics](/admin/visitors) for what's tracked and how to configure it.
...
- **Rotate secrets** — Change your `SECRET_KEY` periodically
