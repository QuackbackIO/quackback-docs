...
- **Ticket linking** -- link support tickets to existing issues, or create a new issue straight from a ticket
- **Status sync** -- update feedback and ticket statuses when GitHub issues are closed or reopened via inbound webhooks
...
Support tickets link to GitHub issues from the ticket detail panel in the inbox:

- **Link existing issue**: paste an issue URL or the `owner/repo#123` shorthand. When the integration pins a repository, issues from other repositories are rejected (inbound sync looks issues up by bare number, so foreign repositories would collide).
- **Create new issue**: creates the issue in the connected repository with the ticket's title, the first customer message as the body, and a link back to the ticket. Internal notes are never included.

Both actions record a team-only note on the ticket thread and require the ticket-assign permission.
...
Tickets have their own mapping table, configured alongside the post mappings in the integration settings. When a linked issue closes or reopens:

- Every linked ticket gets a team-only thread note ("GitHub issue acme/app#412 was closed"), and agents watching the ticket receive a **Linked issue updates** notification. This happens whether or not a mapping applies, so a closing issue never goes unnoticed.
- If a ticket status mapping matches, the ticket moves to the mapped status with the same stage updates and notifications as an agent-driven change.
