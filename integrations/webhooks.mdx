...
Webhooks are managed entirely through the [REST API](/api/introduction). Each webhook specifies:
...
| `conversation.status_changed` | A conversation moves between open, pending, and closed |
...
| `message.note_created` | An agent adds an internal note (private — subscribe only if your endpoint should receive it) |
| `message.deleted` | A public message is deleted |

<Note>
Conversation and message events require the support inbox. Anonymous visitors' emails are never included (synthetic addresses are stripped). System messages (like "chat ended") are represented by `conversation.*` events rather than `message.created`, and internal-note deletions are not emitted.
</Note>
...
<Note>
Board filtering applies only to post and comment events. Conversation and message events aren't tied to a board, so a webhook with `boardIds` set still receives every chat event it subscribed to.
</Note>
