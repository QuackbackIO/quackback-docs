...
Support tickets can create Linear issues from the ticket detail panel in the inbox: **Create new issue** creates the issue in the connected team with the ticket's title and the first customer message as the description. Internal notes are never included.

<Note>
Linking an existing Linear issue by URL is not offered: Linear identifies issues internally by an id a pasted URL doesn't contain, so a hand-entered link couldn't receive status sync. Issues created from Quackback are linked correctly and sync as normal.
</Note>
