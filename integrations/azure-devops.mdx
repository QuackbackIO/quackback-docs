...
### 5. Set Up the Service Hook

Status sync needs an Azure DevOps service hook so work item changes reach Quackback:

1. In Quackback, enable **Status sync** in the Azure DevOps integration settings
2. Copy the **Webhook URL** and **Signing secret** shown below the toggle
3. In Azure DevOps, go to **Project Settings → Service hooks** and create a **Web Hooks** subscription for the **Work item updated** event
4. Paste the webhook URL as the subscription URL and the signing secret as the **Basic authentication password** (leave the username empty)
...
Support tickets link to Azure DevOps work items from the ticket detail panel in the inbox:

- **Link existing work item**: paste a work item URL (`dev.azure.com/org/project/_workitems/edit/123` or the legacy `org.visualstudio.com` form). Work items from other organizations are rejected when the connected organization is known.
- **Create new work item**: creates the work item in the connected project with the ticket's title and the first customer message as the description. Internal notes are never included.
...
2. Quackback verifies the request using Basic Authentication -- the password must match the signing secret from your integration settings
...
Tickets have their own mapping table in the integration settings. Linked tickets also get a team-only thread note and a **Linked issue updates** notification for watching agents on every state change, whether or not a mapping applies.
...
3. **Check authentication** -- the Basic authentication password in the service hook must match the signing secret shown in Quackback
