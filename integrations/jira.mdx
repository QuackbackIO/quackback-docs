...
Support tickets link to Jira issues from the ticket detail panel in the inbox:

- **Link existing issue**: paste a browse URL or the bare `PROJ-42` issue key. When the integration knows your site URL, issues from other Jira sites are rejected.
- **Create new issue**: creates the issue in the connected project with the ticket's title and the first customer message as the description. Internal notes are never included.
