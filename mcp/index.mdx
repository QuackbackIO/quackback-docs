...
- **Work the support inbox**: list and read conversations, reply to visitors, and open or close conversations (when the support inbox is enabled)
...
- **API keys** (`qb_` prefix): Full access to all scopes. Available to team members (admin or member role). Create them in **Admin → Settings → Developers**.
- **OAuth 2.1**: Scoped access. Supports MCP OAuth discovery so clients like Cursor and Claude Code can authenticate automatically without manual key setup.
