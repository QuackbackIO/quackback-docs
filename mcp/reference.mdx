---
title: "MCP Reference"
description: "Complete reference for Quackback's MCP tools and resources. Parameters, return values, and examples for all 39 tools and 6 resources."
icon: "book-open"
order: 2
---
...
| `boardId` | string | - | Filter posts by board TypeID (ignored for changelogs and articles) |
| `categoryId` | string | - | Filter articles by category TypeID (ignored for posts and changelogs) |
| `status` | string | - | Filter by status slug. Posts: `"open"`, `"in_progress"`, etc. Changelogs: `"draft"`, `"published"`, `"scheduled"`, `"all"`. Articles: `"draft"`, `"published"`, `"all"` |
| `tagIds` | string[] | - | Filter posts by tag TypeIDs (ignored for changelogs and articles) |
...
<Note>
Help center tools are only available when the help center feature flag is enabled. See [Help Center](/help-center/setup) for setup.
</Note>
...
```
suggest_post({ conversationId: "conversation_01...", boardId: "board_01...", title: "Add dark mode", content: "Customer asked for a night theme." })
```
...
```
share_post({ conversationId: "conversation_01...", postId: "post_01..." })
```
...
| `stage` | `"received"` \| `"in_progress"` \| `"awaiting_requester"` \| `"resolved"` | - | Filter by customer-facing public stage |
| `requesterPrincipalId` | string | - | Filter to a requester (principal TypeID) |
| `companyId` | string | - | Filter to a company (company TypeID) |
| `sort` | `"recent"` \| `"oldest"` \| `"created"` \| `"priority"` | `"recent"` | Sort order |
...
list_tickets({ companyId: "company_01abc..." })
```
...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `ticketId` | string | - | Ticket TypeID |
| `includeInternal` | boolean | `false` | Include internal teammate notes |
| `cursor` | string | - | Message ID cursor from a previous response, to fetch older messages |

**Scope:** `read:chat` + team role (admin or member)

```
get_ticket({ ticketId: "ticket_01abc...", includeInternal: true })
```
...
| Parameter | Type | Description |
|-----------|------|-------------|
| `type` | `"customer"` \| `"back_office"` \| `"tracker"` | Ticket object type: a requester's request, an internal task, or an umbrella others link to |
| `title` | string | Short summary (max 300 characters) |
| `description` | string | Opening message body, Markdown (GFM), optional (max 10,000 characters) |
| `priority` | `"none"` \| `"low"` \| `"medium"` \| `"high"` \| `"urgent"` | Triage priority (default `"none"`) |
| `requesterPrincipalId` | string | The requester principal TypeID (optional) |
| `companyId` | string | Associated company TypeID (optional) |

**Scope:** `write:chat` + team role (admin or member)

```
create_ticket({ type: "customer", title: "Refund not received", description: "Customer reports a missing refund from last week." })
```
...
|-----------|------|-------------|
| `ticketId` | string | Ticket TypeID |
| `content` | string | Reply text, Markdown (GFM) (1-10,000 characters) |

**Scope:** `write:chat` + team role (admin or member)

```
reply_to_ticket({ ticketId: "ticket_01abc...", content: "We've issued your refund; it should arrive in 3-5 days." })
```
...
|-----------|------|-------------|
| `ticketId` | string | Ticket TypeID |
| `content` | string | Internal note text, Markdown (GFM) (1-10,000 characters) |

**Scope:** `write:chat` + team role (admin or member)

```
add_ticket_note({ ticketId: "ticket_01abc...", content: "Confirmed the refund with billing; awaiting bank processing." })
```
...
| Parameter | Type | Description |
|-----------|------|-------------|
| `trackerTicketId` | string | The tracker ticket TypeID |
| `ticketId` | string | The customer ticket TypeID to track |
...
|-----------|------|-------------|
| `trackerTicketId` | string | The tracker ticket TypeID |
| `ticketId` | string | The linked customer ticket TypeID |
...
|-----|---------|--------|-------|
| `quackback://boards` | All boards | `id`, `name`, `slug` | `read:feedback` |
| `quackback://statuses` | All statuses | `id`, `name`, `slug`, `color` | `read:feedback` |
| `quackback://tags` | All tags | `id`, `name`, `color` | `read:feedback` |
| `quackback://roadmaps` | All roadmaps | `id`, `name`, `slug` | `read:feedback` |
| `quackback://members` | Team members | `id`, `name`, `role` | `read:feedback` |
| `quackback://help-center/categories` | Help center categories with article counts | `id`, `slug`, `name`, `description`, `icon`, `parentId`, `isPublic`, `position`, `articleCount` | `read:article` (team role; help center must be enabled) |
