...
description: "Secure, passwordless authentication for your users and team. Email codes, OAuth, or enterprise SSO."
icon: "lock"
---

# Authentication Overview

No more password resets. Quackback uses passwordless authentication (email codes, OAuth, or enterprise SSO) so users sign in quickly and securely without another password to forget.
...
| OAuth | GitHub, Google |
| SAML SSO | Enterprise identity provider (Enterprise Edition) |
...
3. Configure SAML SSO if using Enterprise Edition

See [Security](/admin/security) for details.

## Enterprise Features

These features are available with the Enterprise Edition (`ee/` packages).

### SAML SSO

Enterprise customers can configure SAML:
- Single Sign-On with identity providers
- Automatic user provisioning
- Centralized access control

See [SSO Setup](/auth/sso) for details.

### SCIM Provisioning

Automatic user management:
- Create users on IdP assignment
- Remove users on de-assignment
- Sync user attributes

<Note>
Enterprise features require `INCLUDE_EE=true` and a valid license.
</Note>
...
3. Check `BETTER_AUTH_URL` matches your domain
...
- [SSO](/auth/sso) - Enterprise SSO (SAML)
