...
description: "Configure passwordless email-based one-time password (OTP) authentication for secure, user-friendly sign-in"
icon: "envelope"
---

# Email OTP Authentication

Quackback uses email-based one-time password (OTP) codes as the primary passwordless authentication method. This approach is secure, user-friendly, and requires no password management.
...
BETTER_AUTH_SECRET="your-32-character-secret"  # Generate with: openssl rand -base64 32
...
2. Find the **Email** toggle under Portal Authentication
...
To prevent abuse, OTP requests are rate-limited:

- **3 requests per minute** per email address
...
- **Keep secrets secure** - Never commit `BETTER_AUTH_SECRET` to version control
...
- [OAuth Providers](/auth/oauth) - Social login with GitHub, Google, Microsoft
- [SSO](/auth/sso) - Enterprise Single Sign-On
