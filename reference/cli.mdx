...
Runs in multi-tenant cloud mode for testing tenant features.
...
```bash
EDITION=self-hosted bun run build
```
...
```bash
INCLUDE_EE=true bun run build
```

<Note>
Enterprise features require a valid license. Contact the Quackback team for licensing information.
</Note>
