...
Comments support nested replies, emoji reactions, and the ability to pin team responses as the "official answer" to a post. Team member comments are automatically badged to distinguish them from community feedback. Team members can also post [private comments](#private-comments) for internal-only discussion.
...
| `isPrivate` | `boolean` | No | Mark as private (team-only). Defaults to `false` |
...
```typescript
// Post a team-only internal note
const result = await createCommentFn({
  data: {
    postId: 'post_01h455vb4pex5vsknk084sn02q',
    content: 'This user is on the enterprise plan. Let\'s prioritize this.',
    isPrivate: true,
  },
})
```
...
- Private comments only notify team member subscribers, not portal users
</Tip>
...
Update a comment as a team member or comment author.

**Function:** `updateCommentFn`

**Method:** `POST`

**Authentication:** Any authenticated user (`admin`, `member`, or `user` role). The service layer checks that the caller is either a team member or the comment author.
...
- Comment must not be private (`isPrivate = false`)
</Note>
...
Team members can post private comments that are only visible to other team members. Private comments are useful for internal discussion - coordinating responses, sharing context, or discussing priorities without the conversation being visible to portal users.

### Behavior

- Only team members (`admin` or `member` role) can create private comments. Setting `isPrivate: true` as a `user` role throws a `ForbiddenError`.
- Replies to private comments are always private. The `isPrivate` flag is automatically inherited from the parent comment.
- Private comments are excluded from the public portal view. They are only returned in admin/team contexts.
- Private comments don't increment the public `commentCount` on the post.
- Private comments cannot be pinned as the official response.
- Notifications for private comments are only sent to team member subscribers, not portal users.
...
// Post a private internal note
const result = await createCommentFn({
  data: {
    postId: 'post_01h455vb4pex5vsknk084sn02q',
    content: 'Customer is on the enterprise plan. Let\'s prioritize this request.',
    isPrivate: true,
  },
})

// Reply to a private comment (automatically private)
const reply = await createCommentFn({
  data: {
    postId: 'post_01h455vb4pex5vsknk084sn02q',
    content: 'Agreed, I\'ll bump it to the top of the sprint.',
    parentId: result.comment.id,
    // isPrivate is inherited from the parent - no need to set it
  },
})
```
...
| `isPrivate` | Optional boolean. Only team members (`admin` or `member`) can set to `true` |
