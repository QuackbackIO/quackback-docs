...
- **Health endpoints** at [`/api/health/live`](/api/health) (process up) and [`/api/health/ready`](/api/health) (database, Redis, migrations, and worker checks)
...
OAuth providers (GitHub, Google, Discord, etc.) and integrations (Slack, etc.) are configured in the admin UI under **Settings → Authentication / Integrations**. See the [Environment Variables Reference](/reference/environment-variables) for the full list.
...
For managed databases (AWS RDS, Supabase, etc.):
...
In production, place a reverse proxy in front of Quackback for HTTPS. See the [Reverse Proxy Configuration](/self-hosting/reverse-proxy) guide for Caddy, Nginx, and Traefik examples.
...
# Readiness: are the database, Redis, migrations, and workers all okay?
curl http://localhost:3000/api/health/ready
```

`docker-compose.prod.yml`'s built-in healthcheck polls `/api/health/ready`. See [`/api/health`](/api/health) for the full response shape.

## Run a separate worker

The single-container default (`QUACKBACK_ROLE=all`) serves HTTP and runs background workers in one process. To scale HTTP and background processing independently, run the same image with `QUACKBACK_ROLE=web` for front-end replicas and `QUACKBACK_ROLE=worker` for a dedicated background-processing replica. See [Scale with multiple replicas](/self-hosting/scaling) for the full guide.
...
- [Scale with multiple replicas](/self-hosting/scaling) - Split web and worker roles
- [Configuration Reference](/reference/environment-variables) - All environment variables
- [Reverse Proxy Setup](/self-hosting/reverse-proxy) - SSL/TLS and proxy configurations
