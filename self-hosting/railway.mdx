...
2. Set the health check path to [`/api/health/ready`](/api/health) (checks database, Redis, migrations, and worker boot status). Use [`/api/health/live`](/api/health) instead if you only want to confirm the process is up.
...
- [Reverse Proxy](/self-hosting/reverse-proxy) - Custom proxy configurations
- [Docker Deployment](/self-hosting/docker) - Alternative deployment with Docker
