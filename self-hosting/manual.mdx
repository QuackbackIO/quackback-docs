...
description: "Install Quackback with Bun and a direct PostgreSQL connection."
icon: "terminal"
---
...
- [Redis](https://redis.io/) 7+ or [Dragonfly](https://www.dragonflydb.io/) as a Redis-compatible alternative
...
<Tip>
One instance handles everything by default (`QUACKBACK_ROLE=all`, the default when the variable is unset). To scale HTTP and background processing separately, add a second systemd unit with `Environment=QUACKBACK_ROLE=worker` (and set the first unit's `QUACKBACK_ROLE=web`). See [Scale with multiple replicas](/self-hosting/scaling).
</Tip>
