...
<Note>
Without [AI configured](/admin/ai-features), search falls back to keyword matching only. It still works, but it won't understand natural language queries or synonyms.
</Note>
...
The same [AI configuration](/admin/ai-features) used for other AI features powers the embeddings — set `AI_EMBEDDING_MODEL` alongside your API key and endpoint. Choose a model that produces 1536-dimensional vectors, such as `text-embedding-3-small`. There's nothing separate to configure for the help center.
