...
Quackback ships a dedicated Canny importer — a command-line tool that connects to the Canny API and migrates posts, votes, comments, users, statuses, tags, and changelog entries automatically. No CSV wrangling needed.

1. Create a Quackback API key in **Admin → Settings → API Keys**, and create your target boards first (the importer matches Canny boards by name).
2. Validate with a dry run, then run it for real:
   ...
   bun scripts/import/cli.ts canny \
     ...
     --dry-run --verbose   # drop --dry-run to import for real
   ```

See [Import and export data](/admin/import-export#migrate-from-canny) for full details.
