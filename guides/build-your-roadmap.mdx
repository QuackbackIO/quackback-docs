...
A roadmap is a derived view: you define a base filter and a set of status columns once, and posts appear automatically as their status changes. There's no separate step to add or remove a post from the roadmap. See [Build a public roadmap](/admin/roadmap) for setup instructions.
...
## Start with a team-only roadmap

If you're not sure what to commit to publicly, use a team-only roadmap first. Stage items internally, discuss with your team, and only widen visibility to public when you're confident they'll ship.

This avoids the awkward situation of a post showing up on a public roadmap and then disappearing. Users notice. It erodes trust faster than not having a roadmap at all.

**The workflow:**
1. During your weekly review, identify posts that should become roadmap items.
2. Move each one to "Planned." It appears on your team-only roadmap automatically, since that's what the base filter matches.
3. Discuss with your team in the next planning session.
4. When committed, switch the roadmap's visibility to public, or create a separate public roadmap with a base filter for "ready to announce" statuses or tags.
...
Default to status columns, not timelines. "Planned" means "we're going to build this." It does not mean "next quarter."
...
<Note>
The one exception: if your team commits to ship dates and wants to publish them, use a [date-periods roadmap](/admin/roadmap#choose-a-layout) instead of adding dates to a status roadmap. It buckets posts by their eta into monthly, quarterly, or semiannual periods, so a missed estimate moves the post to the next period instead of leaving a broken promise sitting on the page.
</Note>

## Linking feedback to roadmap items

Every roadmap item is a feedback post. This connection is the whole point - it shows users that the roadmap is driven by their input.

When a post is ready for the roadmap:
1. Update its status to "Planned." It shows up on the roadmap the moment the status matches your base filter.
2. Leave a comment on the post: "This is now on our roadmap."
3. Subscribers get notified automatically.

When an item moves to "In Progress," update the status on the linked post too. This keeps users informed without you writing individual updates to every voter, and the roadmap column updates itself.
