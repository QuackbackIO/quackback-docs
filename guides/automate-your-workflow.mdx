...
Quackback gives you three ways to automate, and they solve different problems:

- **[Workflows](/automation/workflows)** for in-product support automation: routing conversations, applying SLAs, closing idle tickets. If the trigger and the action both happen inside Quackback, start here.
- **Webhooks and integrations** for syncing external systems: pushing events to Slack, Zapier, or your own server. If the action needs to happen somewhere else, this is the layer for it.
- **[MCP](/mcp)** for AI-assisted triage: an agent that reads feedback and suggests or takes action with your approval. Use it when the judgment call is fuzzy enough that you want an AI in the loop rather than fixed rules.

Most teams end up using all three. A workflow closes an abandoned conversation, a webhook tells Slack about it, and MCP helps you triage the backlog once a week.
...
When built-in integrations don't cover your use case, webhooks let you send feedback events to any HTTP endpoint. This is the layer for syncing to external systems. For automation that stays inside Quackback, like conversation and ticket routing, use [workflows](/automation/workflows) instead.
...
| **As needed** | Webhooks for custom automation, [workflows](/automation/workflows) for support routing and SLAs, MCP for AI triage |
